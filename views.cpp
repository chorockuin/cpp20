...
void view_is_pointer() {
    ...
    // 즉, 아래와 같이 값을 꺼내려고 역참조 할 때 람다 함수가 적용된다
...
void ref_view() {
    ...
    // 아래 두 출력이 똑같이 vector의 첫번째 요소 값인 1을 리턴하고 있다는 얘기는
    // std::ranges::ref_view가 역참조 연산자(*)를 오버라이딩했다는 의미
    ...
    // 근데 왜 아래 처럼 직관적으로 참조를 안쓰고 ref_view를 쓰지? ref_view1()를 보자
    std::vector<int>& r = v;
}
...
void ref_view1() {
    ...
    // 그러나 아래처럼 ref_view를 쓰면 참조 "변수"를 만드는 것과 같다
    // 즉 포인터 변수를 만드는 것과 같다
    ...
    // 포인터만 복사되고, 포인터가 가리키는 값은 복사되지 않는 것과 같다
    r3 = r4;

    // r3, r4는 같은 곳을 가리키지만
    std::cout << r3[0] << std::endl; // 6
    std::cout << r4[0] << std::endl; // 6

    // 원본 v3, v4가 가리키는 곳은 변하지 않았다
    std::cout << v3[0] << std::endl; // 1
    std::cout << v4[0] << std::endl; // 6
}

// take_view를 구현한다면, 위에서 언급했던 동작을 구현해야 한다
template<typename R>
class take_view {
...
void reverse_view() {
    ...
    // 이렇게 std::views::reverse()와 같이 range adaptor object(함수 객체)를 써도 된다
